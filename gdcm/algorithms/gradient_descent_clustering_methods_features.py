                    ...
                    for k in range(self.n_clusters):
                        ...
                        if to_update:
                            ...
                            if self.gdc_vanilla:
                                ...
                                updated_centroid = self.centroids[k, :] - self.step_size * grads
                                ...
                                grads = jax.jacfwd(distance_fn, argnums=(1,))(mean_batch, self.centroids[k, :], self.p)
                                grads = jax.tree_leaves(grads)[0]
                                previous_moment_1 = deepcopy(self.moment_1)  # first moment (estimate) of gradients
                                previous_moment_2 = deepcopy(self.moment_2)  # second moment (square) of gradients
                                tmp_moment_1 = (self.mu_1*previous_moment_1) + (1-self.mu_1)*grads
                                tmp_moment_2 = (self.mu_2 * previous_moment_2) + (1-self.mu_2)*(jnp.power(grads, 2))
                                self.moment_1 = tmp_moment_1/(1-self.mu_1**t)  # m in my notation
                                self.moment_2 = tmp_moment_2/(1-self.mu_2**t)  # _range_len in my notation

                                updated_centroid = self.centroids[k, :] - (
                                        (self.step_size*self.moment_1)/(jnp.sqrt(self.moment_2)+1e-8)
                                )
                                self.centroids = self.centroids.at[k].set(updated_centroid)
                            else:
